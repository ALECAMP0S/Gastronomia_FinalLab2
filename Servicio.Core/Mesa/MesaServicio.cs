...
namespace Servicio.Core.Mesa
{
    public class MesaServicio : IMesaServicio
    {
        ...
        public EstadoMesa ObtenerEstadoMesa(long id)
        {
            using (var context = new ModeloGastronomiaContainer())
            {
                var Mesa = context.Mesas.FirstOrDefault(x => x.Id == id);
                var estadoMesa = Mesa.EstadoMesa;
                return estadoMesa;
            }
        }
        ...
        public IEnumerable<MesaDto> ObtenerMesasOcupadas()
        {
            using (var context = new ModeloGastronomiaContainer())
            {
                var mesas = context.Mesas.AsNoTracking().Where( x=> x.EstadoMesa == EstadoMesa.Ocupada)
                    .ToList();
